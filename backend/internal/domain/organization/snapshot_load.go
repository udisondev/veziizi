...
// битое состояние.
func NewFromStore(id uuid.UUID, snapshotJSON []byte, evts []eventstore.Event) (*Organization, error) {
	...
	if err := json.Unmarshal(snapshotJSON, &snap); err != nil {
		return nil, fmt.Errorf("unmarshal organization snapshot: %w", err)
	}

	org, err := NewFromSnapshot(id, snap)
	if err != nil {
		return nil, err
	}
