...
const (
	...
	TypeCarrierInvited            = "freight_request.carrier_invited"
)
...
func init() {
	...
	eventstore.RegisterEventType[CarrierInvited](TypeCarrierInvited)
}
...
// CarrierInvited is a fire-and-forget event emitted when the customer pings a carrier
// about an active freight request. It triggers a notification rule and is logged
// in freight_request_invites_log for anti-spam (one ping per carrier_org per request).
//
// InviteID is generated by the service before emitting the event so the worker
// can use the same identifier when persisting the projection row — making replays
// fully idempotent (the row is keyed by its own UUID, not regenerated each time).
type CarrierInvited struct {
	eventstore.BaseEvent
	InviteID     uuid.UUID `json:"invite_id"`
	CarrierOrgID uuid.UUID `json:"carrier_org_id"`
	VehicleID    uuid.UUID `json:"vehicle_id"`
	InvitedBy    uuid.UUID `json:"invited_by"` // customer member ID
}

func (e CarrierInvited) EventType() string { return TypeCarrierInvited }
