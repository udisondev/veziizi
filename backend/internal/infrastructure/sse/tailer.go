...
// Tailer хвостом читает Redis-стримы доменных событий: XREAD BLOCK с "$",
// БЕЗ consumer group — каждый инстанс API видит весь поток (broadcast, не
// балансировка), а ack/pending-механика группы здесь не нужна: пропущенные
// события клиент компенсирует refetch'ем при переподключении.
//
