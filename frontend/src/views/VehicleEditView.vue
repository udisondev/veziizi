...
function loadVehicle(v: Vehicle) {
  ...
  form.value = {
    ...
    width: v.width,
    height: v.height,
    loading_types: (v.loading_types ?? []) as LoadingType[],
    requires_adr: v.requires_adr,
    thermograph: v.thermograph,
    has_temperature: !!v.temperature,
    temp_min: v.temperature?.min,
    temp_max: v.temperature?.max,
  }
}
...
async function handleSubmit() {
  if (!auth.organizationId || !vehicle.value) return
...
onMounted(async () => {
  ...
  try {
    const v = await vehiclesApi.get(vehicleId.value)
    loadVehicle(v)
  } catch {
